...
class SparseSingleLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  //create blob -> batch=3; channels=3 (RGB); pixels = 4 (2x2)
  SparseSingleLayerTest()
  ...
  virtual ~SparseSingleLayerTest() { 
    delete blob_bottom_0_;
    delete blob_bottom_1_;
    delete blob_top_;
  }

  Dtype compute_energy(shared_ptr<SparseSingleLayer<Dtype> > layer, LayerParameter layer_param){
    ...
    int num_elements = this->blob_bottom_vec_[1]->shape(1); // M
    Dtype lambda     = layer_param.mutable_sparse_single_param()->lambda();
    Dtype E = 0;
...
TYPED_TEST_CASE(SparseSingleLayerTest, TestDtypesAndDevices);

//TYPED_TEST(SparseSingleLayerTest, TestSetUp) {
//
//  typedef typename TypeParam::Dtype Dtype;
//  LayerParameter layer_param;
//
//  shared_ptr<SparseSingleLayer<Dtype> > layer(
//      new SparseSingleLayer<Dtype>(layer_param));
//
...
//}
//
//TYPED_TEST(SparseSingleLayerTest, TestForward) {
//  typedef typename TypeParam::Dtype Dtype;
...
//    //Set params
//    LayerParameter layer_param;
//    SparseSingleParameter* sparse_single_param =
//        layer_param.mutable_sparse_single_param();
//
//    sparse_single_param->set_lambda(0.1);
//    sparse_single_param->set_eta(0.001);
//    sparse_single_param->set_bias_term(true);
//
//    // Set weights
//    sparse_single_param->mutable_weight_filler()->set_type("uniform");
//    sparse_single_param->mutable_weight_filler()->set_min(0.001);
//    sparse_single_param->mutable_weight_filler()->set_max(1);
//
//    // Set bias
//    sparse_single_param->mutable_bias_filler()->set_type("uniform");
//    sparse_single_param->mutable_bias_filler()->set_min(0.001);
//    sparse_single_param->mutable_bias_filler()->set_max(0.5);
//
//    // Create layer
//    shared_ptr<SparseSingleLayer<Dtype> > layer(
//        new SparseSingleLayer<Dtype>(layer_param));
...
TYPED_TEST(SparseSingleLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  bool IS_VALID_CUDA = false;
  ...
  if (Caffe::mode() == Caffe::CPU ||
    ...
    sparse_single_param->set_eta(2);
    sparse_single_param->set_lambda(0.1);
    sparse_single_param->set_bias_term(true);

    // Set weights
    sparse_single_param->mutable_weight_filler()->set_type("uniform");
    sparse_single_param->mutable_weight_filler()->set_min(0.01);
    sparse_single_param->mutable_weight_filler()->set_max(0.1);

    // Set bias
    sparse_single_param->mutable_bias_filler()->set_type("uniform");
    sparse_single_param->mutable_bias_filler()->set_min(0.1);
    sparse_single_param->mutable_bias_filler()->set_max(1);

    SparseSingleLayer<Dtype> layer(layer_param);
